...
class MatchCog(commands.Cog, name="match"):
    ...
    @app_commands.command(name="bracket", description="View the tournament bracket visually")
    @app_commands.describe(tournament_id="Tournament ID")
    async def bracket(self, interaction: discord.Interaction, tournament_id: str) -> None:
        ...
        from app.services.visuals.brackets import VisualBracketService
        import discord
        from sqlalchemy import select
        ...
        async with AsyncSessionLocal() as session:
            ...
            b_repo = BracketRepository(session)
            bracket_data = await b_repo.get_full_bracket_tree(guild.organization_id, tournament_id)
            
            if "error" in bracket_data:
                await interaction.followup.send(embed=error_embed(bracket_data["error"]), ephemeral=True)
                return

            try:
                vis_svc = VisualBracketService()
                img_buf = await vis_svc.generate_se_bracket_image(tournament.name, bracket_data)
                file = discord.File(fp=img_buf, filename="bracket.png")
                
                embed = discord.Embed(title=f"⚔️ {tournament.name} Bracket", color=discord.Color.blue())
    ...
    async def standings(self, interaction: discord.Interaction, tournament_id: str) -> None:
        ...
        from app.services.visuals.standings import VisualStandingsService
        import discord
        from sqlalchemy import select
        ...
        async with AsyncSessionLocal() as session:
            ...
            # Generate Visual Standings Image
            try:
                vis_svc = VisualStandingsService()
                img_buf = await vis_svc.generate_standings_image(tournament.name, standings_data)
                file = discord.File(fp=img_buf, filename="standings.png")
                
                embed = discord.Embed(title=f"📊 {tournament.name} Standings", color=discord.Color.gold())
                embed.set_image(url="attachment://standings.png")
                
                await interaction.followup.send(embed=embed, file=file)
            except Exception as exc:
                logger.error("Visual standings generation failed: %s", exc)
